...
namespace EsCQRSQuestions.Playwright.Base
{
    public class BaseTest : IDisposable
    {
        ...
        public async Task SetUp()
        {
            ...
            try
            {
                // Kill any existing EsCQRSQuestions processes first
                KillEsCQRSQuestionsProcesses();
                ...
                {
                    throw new DirectoryNotFoundException("Could not find EsCQRSQuestions.AppHost directory in any of the expected locations");
                }
        ...
        public async Task TearDown()
        {
            ...
            // Kill EsCQRSQuestions processes
            Console.WriteLine("Killing EsCQRSQuestions processes...");
            var killSw = Stopwatch.StartNew();
            
            KillEsCQRSQuestionsProcesses();
            
            killSw.Stop();
            Console.WriteLine($"EsCQRSQuestions processes killed in {killSw.ElapsedMilliseconds}ms");
        }
        ...
        private string FindAppHostDirectory(string startingDirectory)
        {
            ...
            {
                "../../EsCQRSQuestions.AppHost",                           // From bin/Debug/net9.0
                "../../../EsCQRSQuestions.AppHost",                        // From bin/Debug
                "../../../../EsCQRSQuestions.AppHost",                     // From bin
                "../../../../../EsCQRSQuestions.AppHost",                  // From Playwright
                "../../..",                                                 // To solution root
            ...
            {
                string appHostPath = Path.Combine(solutionRoot, "EsCQRSQuestions.AppHost");
            ...
            foreach (string relativePath in possiblePaths)
            {
                try
                {
                    ...
                    if (Directory.Exists(fullPath))
                    {
                        ...
                        if (Path.GetFileName(fullPath) == "EsCQRSQuestions.AppHost")
                        {
                            return fullPath;
                        }
                        
                        // Check if AppHost is a subdirectory
                        string appHostSubdir = Path.Combine(fullPath, "EsCQRSQuestions.AppHost");
            ...
            try
            {
                string projectRoot = "/Users/tomohisa/dev/GitHub/Sekiban/templates/Sekiban.Pure.Templates/content/Sekiban.Orleans.Aspire";
                Console.WriteLine($"Trying hardcoded project root: {projectRoot}");
                
                if (Directory.Exists(projectRoot))
                {
                    string appHostPath = Path.Combine(projectRoot, "EsCQRSQuestions.AppHost");
                    if (Directory.Exists(appHostPath))
                    {
                        return appHostPath;
                    }
        ...
        private string FindSolutionRoot(string startingDirectory)
        {
            try
            {
                ...
                // Go up the directory tree looking for EsCQRSQuestions.sln
                while (!string.IsNullOrEmpty(directory))
                {
                    Console.WriteLine($"Checking for solution in: {directory}");
                    
                    // Check if the solution file exists in this directory
                    if (File.Exists(Path.Combine(directory, "EsCQRSQuestions.sln")))
                    {
        ...
        private void KillEsCQRSQuestionsProcesses()
        {
            try
            {
                ...
                // Use pkill to kill all EsCQRSQuestions processes
                var pkillProcess = new Process
                {
                    StartInfo = new ProcessStartInfo
                    {
                        FileName = "pkill",
                        Arguments = "-9 -f \"EsCQRSQuestions.\"",
                ...
                Console.WriteLine("All EsCQRSQuestions processes killed");
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Error in KillEsCQRSQuestionsProcesses: {ex.Message}");
            }
        }
                    ...
                    if (buttonCount > 0)
                    {
                        ...
                        for (int i = 0; i < buttonCount; i++)
                        {
                            ...
                            if (await closeButton.IsVisibleAsync())
                            {
                                ...
                                Console.WriteLine($"Waited 500ms after clicking close button at {modalSw.ElapsedMilliseconds}ms");
                            }
                        }
                    }
                    else

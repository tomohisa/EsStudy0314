...
[GenerateSerializer]
public record StartDisplayCommand(Guid QuestionId) : ICommandWithHandler<StartDisplayCommand>
{
    public static async Task<ResultBox<EventOrNone>> HandleAsync(StartDisplayCommand command, ICommandContext context)
    {
        ...
        {
            return ResultBox.FromException<EventOrNone>(new InvalidOperationException("Question is already being displayed"));
        }

        return EventOrNone.EventWithTags(new QuestionDisplayStarted(), tag, new QuestionGroupTag(question.QuestionGroupId));
    }
}

...
FRONTEND_PATH=$(jq -r '.adminwebRelativePath' "$CONFIG_FILE")

# Verify the Frontend path exists
if [ ! -d "$FRONTEND_PATH" ]; then
    echo "Error: AdminWeb directory not found at $FRONTEND_PATH"
    exit 1
fi

echo "AdminWeb path: $FRONTEND_PATH"

# App Service name will follow the naming convention
APP_SERVICE_NAME="adminweb-${RESOURCE_GROUP}"

...
load_config() {
  ...
  BACKEND_MEMORY="$(jq -r '.backendMemory // "1Gi"' "$CONFIG_FILE")"
  BACKEND_CPU="$(jq -r '.backendCpu // "0.5"' "$CONFIG_FILE")"
  FRONTEND_MIN_REPLICAS="$(jq -r '.frontendMinReplicas // 0' "$CONFIG_FILE")"
  FRONTEND_MAX_REPLICAS="$(jq -r '.frontendMaxReplicas // 2' "$CONFIG_FILE")"
  FRONTEND_MEMORY="$(jq -r '.frontendMemory // "1Gi"' "$CONFIG_FILE")"
  FRONTEND_CPU="$(jq -r '.frontendCpu // "0.5"' "$CONFIG_FILE")"
